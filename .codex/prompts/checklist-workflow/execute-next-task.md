...
You must **favor concrete action** over mere description whenever environment constraints allow.
...
1. Run targeted validations:
   - `uv run pytest` for affected tests or directories (if reasonable).
   - `uv run ruff check` or other relevant checks when you have edited Python code.
2. Capture results:
...
1. Modify the checklist file:
   ...
   - Add or adjust file paths, tests, and Branch/PR references if they became concrete.
