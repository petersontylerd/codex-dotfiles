...
### Command variables
- `create-initiative-plan`: `$OPTIMIZED_PROMPT_PATH` (filesystem path to the narrative file containing goals, principles, constraints; load and use the file contents) used to infer initiative name and propose branch slug.
...
### Command cheatsheet
- `create-initiative-plan`: Start or reorient an initiative. Outputs goals, constraints, branch suggestion, and initial plan (driven by the contents of the file at `$OPTIMIZED_PROMPT_PATH`).
