...
def validate_feature_id_consistency(feature_file: Path, artifact: Dict[str, Any]) -> List[str]:
    ...
    if feature_file.name != expected_filename:
        errors.append(f"Feature filename should be {expected_filename}, got {feature_file.name}")
    ...
    if 'metadata' in artifact:
        ...
        if 'feature_id' in metadata:
            ...
            if actual_feature_id != expected_feature_id:
                errors.append(f"Feature ID in metadata ({actual_feature_id}) doesn't match directory ({expected_feature_id})")
        
        if 'parent_epic' in metadata:
            actual_parent_epic = metadata['parent_epic']
            if actual_parent_epic != expected_epic_id:
                errors.append(f"Parent epic in metadata ({actual_parent_epic}) doesn't match directory ({expected_epic_id})")
...
def validate_features() -> bool:
    ...
    script_dir = Path(__file__).parent.parent.parent.parent
    template_path = script_dir / "templates" / "product-plan" / "development" / "feature.yaml"
    ...
    for feature_file in feature_files:
        ...
        if artifact_errors:
            all_errors.extend([f"{feature_file.name}: {error}" for error in artifact_errors])
            continue
        
        # Validate structure
        structure_errors = validate_structure(artifact, template)
        if structure_errors:
            all_errors.extend([f"{feature_file.name}: {error}" for error in structure_errors])
