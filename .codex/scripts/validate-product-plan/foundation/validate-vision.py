...
"""
ABOUTME: Validates vision.yaml artifact against its template schema
Ensures the vision artifact follows the expected structure and contains required fields
"""
...
def validate_vision() -> bool:
    """Main validation function for vision.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "vision.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "vision.yaml"
    
    print(f"{BLUE}Validating vision.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ vision.yaml validation passed{RESET}")
        return True

if __name__ == "__main__":
    success = validate_vision()
