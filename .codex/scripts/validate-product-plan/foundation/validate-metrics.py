...
"""
ABOUTME: Validates metrics.yaml artifact against its template schema
Ensures the metrics artifact follows the expected structure and contains required fields
"""
...
def validate_metrics() -> bool:
    """Main validation function for metrics.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "metrics.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "metrics.yaml"
    
    print(f"{BLUE}Validating metrics.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ metrics.yaml validation passed{RESET}")
        return True
