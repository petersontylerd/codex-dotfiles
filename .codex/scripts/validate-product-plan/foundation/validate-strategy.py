...
"""
ABOUTME: Validates strategy.yaml artifact against its template schema
Ensures the strategy artifact follows the expected structure and contains required fields
"""
...
def validate_strategy() -> bool:
    """Main validation function for strategy.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "strategy.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "strategy.yaml"
    
    print(f"{BLUE}Validating strategy.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ strategy.yaml validation passed{RESET}")
        return True
