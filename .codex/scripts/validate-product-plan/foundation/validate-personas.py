...
"""
ABOUTME: Validates personas.yaml artifact against its template schema
Ensures the personas artifact follows the expected structure and contains required fields
"""
...
def validate_personas() -> bool:
    """Main validation function for personas.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "personas.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "personas.yaml"
    
    print(f"{BLUE}Validating personas.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ personas.yaml validation passed{RESET}")
        return True

if __name__ == "__main__":
    success = validate_personas()
