...
"""
ABOUTME: Validates brainstorm.yaml artifact against its template schema
Ensures the brainstorm artifact follows the expected structure and contains required fields
"""
...
def validate_brainstorm() -> bool:
    """Main validation function for brainstorm.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "brainstorm.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "brainstorm.yaml"
    
    print(f"{BLUE}Validating brainstorm.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ brainstorm.yaml validation passed{RESET}")
        return True

if __name__ == "__main__":
    success = validate_brainstorm()
