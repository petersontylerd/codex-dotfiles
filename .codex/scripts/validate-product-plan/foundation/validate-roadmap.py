...
"""
ABOUTME: Validates roadmap.yaml artifact against its template schema
Ensures the roadmap artifact follows the expected structure and contains required fields
"""
...
def validate_roadmap() -> bool:
    """Main validation function for roadmap.yaml."""
    # Define paths
    script_dir = Path(__file__).parent.parent.parent.parent
    artifact_path = script_dir / "product-plan" / "foundation" / "roadmap.yaml"
    template_path = script_dir / "templates" / "product-plan" / "foundation" / "roadmap.yaml"
    
    print(f"{BLUE}Validating roadmap.yaml...{RESET}")
    ...
    if structure_errors:
        ...
        print(f"{GREEN}✓ roadmap.yaml validation passed{RESET}")
        return True

if __name__ == "__main__":
    success = validate_roadmap()
